...
public class CalcFeeResource {
	...
	private static final Logger LOGGER = LogManager.getLogger(CalcFeeResource.class);
	...
	public String calcFeeForArbitraryTransaction(String x) {
		try {
			...
			// CALC FEE
			Pair<BigDecimal, Integer> result = Controller.getInstance()
					.calcRecommendedFeeForArbitraryTransaction(dataBytes, payments);
	...
	public String calcFeeForNameReg(String x) {
		try {
			...
			jsonObject = new JSONObject();
			Pair<BigDecimal, Integer> result = Controller.getInstance().calcRecommendedFeeForNameRegistration(name,
					value);
			jsonObject.put("fee", result.getA().toPlainString());
			jsonObject.put("length", result.getB());
			return jsonObject.toJSONString();
	...
	public String calcFeeForBlogPost(String x) {
		try {
			...
			if (StringUtil.isBlank(body)) {
				throw ApiErrorFactory.getInstance().createError(ApiErrorFactory.ERROR_BODY_EMPTY);
			}
			...
			// CALC FEE
			Pair<BigDecimal, Integer> result = Controller.getInstance().calcRecommendedFeeForArbitraryTransaction(
					dataStructure.toJSONString().getBytes(StandardCharsets.UTF_8), null);
