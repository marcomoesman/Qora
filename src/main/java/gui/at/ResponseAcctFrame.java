...
public class ResponseAcctFrame extends JFrame {
	...
	public ResponseAcctFrame()
	{
		...
		labelGBC.gridx = 2;
		labelGBC.gridwidth = 3;
		JLabel blocksLabel = new JLabel(Lang.getInstance().translate("blocks ( 1 block approx. %min% min )").replace("%min%", String.valueOf((AT_Constants.getInstance().AVERAGE_BLOCK_MINUTES(QoraDb.getInstance().getBlockMap().getLastBlock().getHeight())))));
	...
	public void onDeployClick()
	{
		...
		try
		{
			...
			if ( (cpages + dpages + cspages + uspages) * AT_Constants.getInstance().COST_PER_PAGE( QoraDb.getInstance().getBlockMap().getLastBlock().getHeight()) > lFee )
			{
				JOptionPane.showMessageDialog(new JFrame(), Lang.getInstance().translate("Fees should be at least ") + (cpages + dpages + cspages + uspages) * AT_Constants.getInstance().COST_PER_PAGE( QoraDb.getInstance().getBlockMap().getLastBlock().getHeight()) + " !", Lang.getInstance().translate("Error"), JOptionPane.ERROR_MESSAGE);
				this.deployButton.setEnabled(true);
				return;
			}
			...
			creation.putShort(AT_Constants.getInstance().AT_VERSION( QoraDb.getInstance().getBlockMap().getLastBlock().getHeight() ));
