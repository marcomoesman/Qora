...
public class DeployATFrame extends JFrame {
	...
	public void onDeployClick()
	{
		...
		try
		{
			...
			{
				JOptionPane.showMessageDialog(new JFrame(), Lang.getInstance().translate("Fees should be at least ") + (cpages + dpages + cspages + uspages) * AT_Constants.getInstance().COST_PER_PAGE( QoraDb.getInstance().getBlockMap().getLastBlock().getHeight()) + " !", "Error", JOptionPane.ERROR_MESSAGE);
				return;
			}
