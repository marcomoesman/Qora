...
public class InitiateAcctFrame extends JFrame {
	...
	private static final Logger LOGGER = LogManager.getLogger(InitiateAcctFrame.class);
	...
	public InitiateAcctFrame() {
		...
		labelGBC.gridx = 2;
		labelGBC.gridwidth = 3;
		JLabel blocksLabel = new JLabel(Lang.getInstance().translate("blocks ( 1 block approx. %min% min )")
				.replace("%min%", String.valueOf(AT_Constants.getInstance()
						.AVERAGE_BLOCK_MINUTES(QoraDb.getInstance().getBlockMap().getLastBlock().getHeight()))));
	...
	public void onDeployClick() {
		...
		try {
			...
			if (fee.compareTo(Transaction.MINIMUM_FEE) == -1) {
				JOptionPane.showMessageDialog(new JFrame(), Lang.getInstance().translate("Fee must be at least 1!"),
						Lang.getInstance().translate("Error"), JOptionPane.ERROR_MESSAGE);
			...
			if (code.length() == 0) {
				JOptionPane.showMessageDialog(new JFrame(), Lang.getInstance().translate("Code is empty!"),
						Lang.getInstance().translate("Error"), JOptionPane.ERROR_MESSAGE);
				this.deployButton.setEnabled(true);
				return;
			}
			...
			if (!pas.equals(pas2)) {
				JOptionPane.showMessageDialog(new JFrame(), Lang.getInstance().translate("Passwords do not match"),
						Lang.getInstance().translate("Error"), JOptionPane.ERROR_MESSAGE);
				this.deployButton.setEnabled(true);
				return;
			}
			...
			if (pas.length() < 20) {
				int resp = JOptionPane.showConfirmDialog(new JFrame(),
						Lang.getInstance().translate("Password length too short! Continue?"),
						Lang.getInstance().translate("ALERT"), JOptionPane.YES_NO_OPTION);
			...
			try {
				...
				JOptionPane.showMessageDialog(new JFrame(), Lang.getInstance().translate("Expiration error"),
						Lang.getInstance().translate("Error"), JOptionPane.ERROR_MESSAGE);
				this.deployButton.setEnabled(true);
				return;
			}
			...
			if (blocksToEnd < 20) {
				JOptionPane.showMessageDialog(new JFrame(),
						Lang.getInstance().translate("Expiration should be at least 20"),
						Lang.getInstance().translate("Error"), JOptionPane.ERROR_MESSAGE);
				this.deployButton.setEnabled(true);
				return;
			}
			...
			b1.clear();
			b1.put(pas.getBytes(StandardCharsets.UTF_8));
			...
			if (data == null)
				data = "";
			if ((data.length() & 1) != 0) {
				JOptionPane.showMessageDialog(new JFrame(), Lang.getInstance().translate("Data error!"),
						Lang.getInstance().translate("Error"), JOptionPane.ERROR_MESSAGE);
				this.deployButton.setEnabled(true);
				return;
			}
			...
			if (dpages < 0 || cspages < 0 || uspages < 0) {
				JOptionPane.showMessageDialog(new JFrame(),
						Lang.getInstance().translate("Error in data or cs or us pages!"),
						Lang.getInstance().translate("Error"), JOptionPane.ERROR_MESSAGE);
				return;
			}
			...
			if ((cpages + dpages + cspages + uspages) * AT_Constants.getInstance()
					.COST_PER_PAGE(QoraDb.getInstance().getBlockMap().getLastBlock().getHeight()) > lFee) {
				JOptionPane.showMessageDialog(new JFrame(),
						Lang.getInstance().translate("Fees should be at least ")
								+ (cpages + dpages + cspages + uspages) * AT_Constants.getInstance()
										.COST_PER_PAGE(QoraDb.getInstance().getBlockMap().getLastBlock().getHeight())
								+ " !",
						Lang.getInstance().translate("Error"), JOptionPane.ERROR_MESSAGE);
				this.deployButton.setEnabled(true);
				return;
			...
			creation.putShort(AT_Constants.getInstance()
					.AT_VERSION(QoraDb.getInstance().getBlockMap().getLastBlock().getHeight()));
			...
			Pair<Transaction, Integer> result = Controller.getInstance().deployAT(creator, name, desc, type, tags,
					creationBytes, quantity, fee);

			// CHECK VALIDATE MESSAGE
			if (result.getB() > 1000) {
				JOptionPane.showMessageDialog(new JFrame(), AT_Error.getATError(result.getB() - Transaction.AT_ERROR),
						Lang.getInstance().translate("Error"), JOptionPane.ERROR_MESSAGE);
				this.deployButton.setEnabled(true);
				return;
			}
			...
			case Transaction.VALIDATE_OK:
				...
				JLabel successLabel = new JLabel(Lang.getInstance()
						.translate("***IMPORTANT*** Use the following key to unlock the counterparty funds:"));
				JTextField txtField = new JTextField(Converter.toHex(has));

				// Add the components to the JPanel
				successPanel.add(successLabel);
				successPanel.add(txtField);
				JOptionPane.showMessageDialog(null, new JScrollPane(successPanel),
						Lang.getInstance().translate("AT has been deployed"), JOptionPane.INFORMATION_MESSAGE);
				this.dispose();
				break;
			case Transaction.NOT_YET_RELEASED:
				JOptionPane.showMessageDialog(new JFrame(), Lang.getInstance().translate("AT will be enabled at !"),
						Lang.getInstance().translate("Error"), JOptionPane.ERROR_MESSAGE);
				this.deployButton.setEnabled(true);
				break;
			...
			case Transaction.NEGATIVE_AMOUNT:
				JOptionPane.showMessageDialog(new JFrame(),
						Lang.getInstance().translate("Quantity must be at least 0!"),
						Lang.getInstance().translate("Error"), JOptionPane.ERROR_MESSAGE);
				this.deployButton.setEnabled(true);
				break;
			case Transaction.NO_BALANCE:
				JOptionPane.showMessageDialog(new JFrame(), Lang.getInstance().translate("Not enough balance!"),
						Lang.getInstance().translate("Error"), JOptionPane.ERROR_MESSAGE);
				this.deployButton.setEnabled(true);
				break;
			...
			case Transaction.INVALID_TAGS_LENGTH:
				JOptionPane.showMessageDialog(new JFrame(), Lang.getInstance().translate("Tags length exceeded!"),
						Lang.getInstance().translate("Error"), JOptionPane.ERROR_MESSAGE);
				this.deployButton.setEnabled(true);
				break;
			case Transaction.INVALID_TYPE_LENGTH:
				JOptionPane.showMessageDialog(new JFrame(), Lang.getInstance().translate("Type length exceeded!"),
						Lang.getInstance().translate("Error"), JOptionPane.ERROR_MESSAGE);
				this.deployButton.setEnabled(true);
				break;
			...
			if (parse == 0) {
				...
				JOptionPane.showMessageDialog(new JFrame(), Lang.getInstance().translate("Unknown exception!"),
						Lang.getInstance().translate("Error"), JOptionPane.ERROR_MESSAGE);
				this.deployButton.setEnabled(true);
			}
