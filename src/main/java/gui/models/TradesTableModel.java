...
public class TradesTableModel extends QoraTableModel<Tuple2<BigInteger, BigInteger>, Trade> implements Observer
{
	...
	private void totalCalc()
	{
		...
		{
			String type = tradePair.getB().getInitiatorOrder(QoraDb.getInstance()).getHave() == this.have.getKey() ? "Sell" : "Buy";
	...
	public Object getValueAt(int row, int column) 
	{
		...
		if(row < this.trades.size())
		{
			...
			type = trade.getInitiatorOrder(QoraDb.getInstance()).getHave() == this.have.getKey() ? "Sell" : "Buy";
		}
