...
public class MessageTransactionDetailsFrame extends JFrame {
	...
	private static final Logger LOGGER = LogManager.getLogger(MessageTransactionDetailsFrame.class);
	...
	public MessageTransactionDetailsFrame(final MessageTransaction messageTransaction) {
		...
		messageText = new JTextField(
				(messageTransaction.isText()) ? new String(messageTransaction.getData(), StandardCharsets.UTF_8)
						: Converter.toHex(messageTransaction.getData()));
		...
		encrypted.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (!encrypted.isSelected()) {
					...
					Account account = Controller.getInstance()
							.getAccountByAddress(messageTransaction.getSender().getAddress());
					...
					if (account == null) {
						PrivateKeyAccount accountRecipient = Controller.getInstance()
								.getPrivateKeyAccountByAddress(messageTransaction.getRecipient().getAddress());
					...
					// IF SENDER ME
						...
						publicKey = Controller.getInstance()
								.getPublicKeyByAddress(messageTransaction.getRecipient().getAddress());
					}

					try {
						messageText.setText(new String(
								AEScrypto.dataDecrypt(messageTransaction.getData(), privateKey, publicKey), "UTF-8"));
