...
public class NetworkStatus extends JLabel implements Observer {
	...
	public NetworkStatus() {
		...
		this.okIcon = this.createIcon(Color.GREEN);
		...
		this.addMouseListener(new MouseAdapter() {
			public void mouseEntered(MouseEvent mEvt) {
				if (Controller.getInstance().getStatus() == Controller.STATUS_OK
						|| Controller.getInstance().getStatus() == Controller.STATUS_NO_CONNECTIONS) {
					setToolTipText(
							Lang.getInstance().translate("Block height") + ": " + Controller.getInstance().getHeight()
									+ " (" + Controller.getInstance().getActivePeers().size() + " connections)");
				} else if (Controller.getInstance().getWalletSyncHeight() > 0) {
					setToolTipText(Lang.getInstance().translate("Block height") + ": " + currentHeight + "/"
							+ Controller.getInstance().getHeight() + "/" + Controller.getInstance().getMaxPeerHeight()
							+ " (" + Controller.getInstance().getActivePeers().size() + " connections)");
				} else {
					setToolTipText(Lang.getInstance().translate("Block height") + ": " + currentHeight + "/"
							+ Controller.getInstance().getMaxPeerHeight() + " ("
							+ Controller.getInstance().getActivePeers().size() + " connections)");
				}
			}
		});
	...
	public void update(Observable arg0, Object arg1) {
		...
		if (message.getType() == ObserverMessage.WALLET_SYNC_STATUS) {
			...
			if (currentHeight == -1) {
				this.update(null,
						new ObserverMessage(ObserverMessage.NETWORK_STATUS, Controller.getInstance().getStatus()));
				currentHeight = Controller.getInstance().getHeight();
				return;
			}

			this.setIcon(walletSynchronizingIcon);
			this.setText(Lang.getInstance().translate("Wallet Synchronizing") + " "
					+ Math.round(100 * (currentHeight / Controller.getInstance().getHeight())) + "%");
		}
		...
		if (message.getType() == ObserverMessage.BLOCKCHAIN_SYNC_STATUS) {
			...
			if (Controller.getInstance().getStatus() == Controller.STATUS_SYNCHRONIZING) {
				this.setText(Lang.getInstance().translate("Synchronizing") + " "
						+ Math.round(100 * (currentHeight / Controller.getInstance().getMaxPeerHeight())) + "%");
			}
		}
