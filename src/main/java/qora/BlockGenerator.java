...
public final class BlockGenerator extends Thread implements Observer {
		...
		new Thread() {
			...
			public void run() {
				...
				while (!Controller.getInstance().doesWalletExists()
						|| !Controller.getInstance().doesWalletDatabaseExists()) {
					try {
						Thread.sleep(250);
	...
	public void addUnconfirmedTransaction(Transaction transaction) {
		this.addUnconfirmedTransaction(QoraDb.getInstance(), transaction, true);
	}

	public void addUnconfirmedTransaction(QoraDb db, Transaction transaction, boolean process) {
		// ADD TO TRANSACTION DATABASE
		db.getTransactionMap().add(transaction);
	}

	public List<Transaction> getUnconfirmedTransactions() {
		return new ArrayList<Transaction>(QoraDb.getInstance().getTransactionMap().getValues());
	}
	...
	@Override
	public void run() {
		while (!this.stopping && !QoraDb.getInstance().isStopped()) {
			...
			if (forgingStatus == ForgingStatus.FORGING) {
				...
				if (this.solvingBlock == null || !Arrays.equals(this.solvingBlock.getSignature(), lastBlockSignature)) {
					...
					this.solvingBlock = QoraDb.getInstance().getBlockMap().getLastBlock();
						...
						for (PrivateKeyAccount account : knownAccounts) {
							if (account.getGeneratingBalance().compareTo(BigDecimal.ONE) >= 0) {
								...
								if (!this.blocks.containsKey(account)) {
									...
									this.blocks.put(account,
											this.generateNextBlock(QoraDb.getInstance(), account, this.solvingBlock));
								}
							}
						}
					}
				...
				for (PrivateKeyAccount account : this.blocks.keySet()) {
					...
					/*
					 * Date date = new Date(block.getTimestamp()); DateFormat format =
					 * DateFormat.getDateTimeInstance(); System.out.println(format.format(date));
					 */

					// CHECK IF BLACK TIMESTAMP IS VALID
					if (block.getTimestamp() <= NTP.getTime() && !validBlockFound) {
						// ADD TRANSACTIONS
						this.addUnconfirmedTransactions(QoraDb.getInstance(), block);
	...
	public Block generateNextBlock(QoraDb db, PrivateKeyAccount account, Block block) {
		...
		if (version > 1) {
			AT_Block atBlock = AT_Controller.getCurrentBlockATs(
					AT_Constants.getInstance().MAX_PAYLOAD_FOR_BLOCK(block.getHeight()), block.getHeight() + 1);
			byte[] atBytes = atBlock.getBytesForBlock();
			newBlock = BlockFactory.getInstance().create(version, block.getSignature(), timestamp.longValue(),
					getNextBlockGeneratingBalance(db, block), account, signature, atBytes, atBlock.getTotalFees());
		} else {
			newBlock = BlockFactory.getInstance().create(version, block.getSignature(), timestamp.longValue(),
					getNextBlockGeneratingBalance(db, block), account, signature);
		}
		return newBlock;
	}

	private byte[] calculateSignature(QoraDb db, Block solvingBlock, PrivateKeyAccount account) {
		byte[] data = new byte[0];

		// WRITE PARENT GENERATOR SIGNATURE
		byte[] generatorSignature = Bytes.ensureCapacity(solvingBlock.getGeneratorSignature(),
				Block.GENERATOR_SIGNATURE_LENGTH, 0);
	...
	public void addUnconfirmedTransactions(QoraDb db, Block block) {
		long totalBytes = 0;
		boolean transactionProcessed;

		// CREATE FORK OF GIVEN DATABASE
		QoraDb newBlockDb = db.fork();
		...
		do {
			...
			for (Transaction transaction : orderedTransactions) {
				...
				if (transaction.getTimestamp() <= block.getTimestamp()
						&& transaction.getDeadline() > block.getTimestamp()) {
					try {
						// CHECK IF VALID
	...
	/*
	 * public void addObserver(Observer o) { o.update(null, new
	 * ObserverMessage(ObserverMessage.LIST_TRANSACTION_TYPE,
	 * DBSet.getInstance().getTransactionMap().getValues())); }
	 */

	public static long getNextBlockGeneratingBalance(QoraDb db, Block block) {
