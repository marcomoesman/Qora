...
public class TransferAssetTransaction extends Transaction {
	...
	{
		return generateSignature(QoraDb.getInstance(), sender, recipient, key, amount, fee, timestamp);
	}
	
	public static byte[] generateSignature(QoraDb db, PrivateKeyAccount sender, Account recipient, long key, BigDecimal amount, BigDecimal fee, long timestamp) 
	{
		byte[] data = new byte[0];
