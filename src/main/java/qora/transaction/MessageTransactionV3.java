...
public class MessageTransactionV3 extends MessageTransaction {
	...
	{
		return generateSignature(QoraDb.getInstance(), creator, recipient, key, amount, fee, arbitraryData, isText, encrypted, timestamp);
	}
	
	public static byte[] generateSignature(QoraDb db, PrivateKeyAccount creator, Account recipient, long key, BigDecimal amount, BigDecimal fee, byte[] arbitraryData,byte[] isText, byte[] encrypted, long timestamp) 
	{
		byte[] data = new byte[0];
