...
public class CancelOrderTransaction extends Transaction
{
	...
	public Map<String, Map<Long, BigDecimal>> getAssetAmount() 
	{
		...
		if(QoraDb.getInstance().getCompletedOrderMap().contains(this.order))
		{
			order =  QoraDb.getInstance().getCompletedOrderMap().get(this.order);
		}
		else
		{
			order =  QoraDb.getInstance().getOrderMap().get(this.order);
		}	
	...
	public static byte[] generateSignature(QoraDb db, PrivateKeyAccount creator, BigInteger order, BigDecimal fee, long timestamp) 
	{
		byte[] data = new byte[0];
