...
public class Block {
	...
	public Block(int version, byte[] reference, long timestamp, long generatingBalance, PublicKeyAccount generator,
		...
		if (this.transactions == null) {
			...
			try {
				...
				for (int i = 0; i < transactionCount; i++) {
					...
					byte[] transactionLengthBytes = Arrays.copyOfRange(this.rawTransactions, position,
							position + TRANSACTION_SIZE_LENGTH);
					int transactionLength = Ints.fromByteArray(transactionLengthBytes);

					// PARSE TRANSACTION
					byte[] transactionBytes = Arrays.copyOfRange(this.rawTransactions,
							position + TRANSACTION_SIZE_LENGTH, position + TRANSACTION_SIZE_LENGTH + transactionLength);
	...
	public Block getParent() {
		return this.getParent(QoraDb.getInstance());
	}

	public Block getParent(QoraDb db) {
		return db.getBlockMap().get(this.reference);
	}

	public Block getChild() {
		return this.getChild(QoraDb.getInstance());
	}

	public Block getChild(QoraDb db) {
		return db.getChildMap().get(this);
	}

	public int getHeight() {
		return this.getHeight(QoraDb.getInstance());
	}

	public int getHeight(QoraDb db) {
	...
	public static Block parse(byte[] data) throws Exception {
		...
		if (version > 1) {
			...
			block = new Block(version, reference, timestamp, generatingBalance, generator, generatorSignature, atBytes,
					atFeesL);
		} else {
	...
	public byte[] getProofHash() {
		if (this.version < 3) {
			...
			final byte[] data = Bytes.concat(this.reference, generator.getPublicKey());
	...
	public boolean isSignatureValid() {
		...
		final byte[] generatorSignature = Arrays.copyOfRange(this.reference, 0, GENERATOR_SIGNATURE_LENGTH);
		data = Bytes.concat(data, generatorSignature);

		// WRITE GENERATING BALANCE
		final byte[] baseTargetBytes = Longs.toByteArray(this.generatingBalance);
		data = Bytes.concat(data, baseTargetBytes);

		// WRITE GENERATOR
		final byte[] generatorBytes = Bytes.ensureCapacity(this.generator.getPublicKey(), GENERATOR_LENGTH, 0);
		data = Bytes.concat(data, generatorBytes);
	...
	public int getNextBlockVersion(final QoraDb db) {
		final int height = getHeight(db);
	...
	public boolean isValid() {
		return isValid(QoraDb.getInstance());
	}

	public boolean isValid(final QoraDb db) {
		// CHECK IF PARENT EXISTS
		if (this.reference == null || getParent(db) == null) {
			return false;
		}

		// Check parent doesn't already have a child
		if (getParent(db).getChild(db) != null) {
			return false;
		}

		// CHECK IF TIMESTAMP IS VALID -500 MS ERROR MARGIN TIME
		if (this.timestamp - 500 > NTP.getTime() || this.timestamp < getParent(db).timestamp) {
			return false;
		}

		// CHECK IF TIMESTAMP REST SAME AS PARENT TIMESTAMP REST
		if (this.timestamp % 1000 != getParent(db).timestamp % 1000) {
			return false;
		}

		// CHECK IF GENERATING BALANCE IS CORRECT
		if (this.generatingBalance != BlockGenerator.getNextBlockGeneratingBalance(db, getParent(db))) {
			return false;
		}

		// CHECK IF VERSION IS CORRECT
		if (this.version != getParent(db).getNextBlockVersion(db)) {
			return false;
		}
		...
		final BigInteger baseTarget = BigInteger.valueOf(BlockGenerator.getBaseTarget(this.generatingBalance));
		...
		final long guesses = (this.timestamp - this.getParent(db).getTimestamp()) / 1000;
		final BigInteger lowerTarget = target.multiply(BigInteger.valueOf(guesses - 1));
		target = target.multiply(BigInteger.valueOf(guesses));

		// CONVERT PROOF HASH TO BIGINT
		final BigInteger hashValue = new BigInteger(1, getProofHash());
		...
		if (this.atBytes != null && this.atBytes.length > 0) {
			try {
				final AT_Block atBlock = AT_Controller.validateATs(this.getBlockATs(),
						db.getBlockMap().getLastBlock().getHeight(db) + 1, db);
		...
		for (final Transaction transaction : this.getTransactions()) {
			...
			if (transaction instanceof DeployATTransaction) {
				final DeployATTransaction atTx = (DeployATTransaction) transaction;
			...
			// PROCESS TRANSACTION IN MEMORYDB TO MAKE SURE OTHER TRANSACTIONS VALIDATE
			// PROPERLY
			try {
				transaction.process(fork);
			} catch (Exception e) {
				LOGGER.error("Exception during transaction processing, tx " + Base58.encode(transaction.getSignature()),
						e);
				return false;
			}
		}
	...
	public void process() {
		process(QoraDb.getInstance());
	}

	public void process(final QoraDb db) {
		...
		final List<Transaction> unconfirmedTransactions = new ArrayList<Transaction>(db.getTransactionMap().getValues());
		for (final Transaction transaction : unconfirmedTransactions) {
		...
		// PROCESS FEE
		final BigDecimal blockFee = this.getTotalFee();
		...
		final Block parent = this.getParent(db);
		int height = 1;
		if (parent != null) {
		...
		if (db == QoraDb.getInstance()) {
			LOGGER.debug("Processed block " + height);
		}
	}

	public void orphan() {
		orphan(QoraDb.getInstance());
	}

	public void orphan(QoraDb db) {
		// ORPHAN AT TRANSACTIONS
		LinkedHashMap<Tuple2<Integer, Integer>, AT_Transaction> atTxs = QoraDb.getInstance().getATTransactionMap()
				.getATTransactions(this.getHeight(db));
		...
		while (iter.hasNext()) {
			...
			if (key.getRecipientId() != null && !Arrays.equals(key.getRecipientId(), new byte[AT_Constants.AT_ID_SIZE])
					&& !key.getRecipient().equalsIgnoreCase("1")) {
				Account recipient = new Account(key.getRecipient());
				recipient.setConfirmedBalance(recipient.getConfirmedBalance(db).subtract(BigDecimal.valueOf(amount, 8)),
						db);
	...
	private void orphanTransactions(final List<Transaction> transactions, final QoraDb db) {
	...
	public int getTransactionSeq(final byte[] signature) {
		int seq = 1;
		for (final Transaction transaction : this.getTransactions()) {
			if (Arrays.equals(transaction.getSignature(), signature)) {
				return seq;
	...
	public boolean equals(Object otherObject) {
		if (otherObject instanceof Block) {
			final Block otherBlock = (Block) otherObject;
	...
	public boolean passesCheckpoints() {
		if (Blockchain.CHECKPOINTS.containsKey(getHeight())) {
			return Arrays.equals(Base58.decode(Blockchain.CHECKPOINTS.get(getHeight())), getSignature());
		}
		return true;
	}
}
