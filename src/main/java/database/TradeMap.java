...
public class TradeMap extends DbMap<Tuple2<BigInteger, BigInteger>, Trade> {
	...
	public TradeMap(QoraDb databaseSet, DB database) {
	...
	private Map<Tuple2<BigInteger, BigInteger>, Trade> openMap(final DB database) {
		...
		BTreeMap<Tuple2<BigInteger, BigInteger>, Trade> map = database.createTreeMap("trades")
				.valueSerializer(new TradeSerializer()).makeOrGet();

		// CHECK IF NOT MEMORY DATABASE
		if (databaseSet != null) {
			// PAIR KEY
			this.pairKeyMap = database.createTreeMap("trades_key_pair").comparator(Fun.COMPARATOR).makeOrGet();

			// BIND PAIR KEY
			Bind.secondaryKey(map, this.pairKeyMap,
					new Fun.Function2<Tuple3<String, Long, Tuple2<BigInteger, BigInteger>>, Tuple2<BigInteger, BigInteger>, Trade>() {
						@Override
						public Tuple3<String, Long, Tuple2<BigInteger, BigInteger>> run(
								Tuple2<BigInteger, BigInteger> key, Trade value) {
							Order order = value.getInitiatorOrder((QoraDb) databaseSet);
							...
							return new Tuple3<String, Long, Tuple2<BigInteger, BigInteger>>(pairKey,
									Long.MAX_VALUE - value.getTimestamp(), key);
						}
					});

			//
			this.wantKeyMap = database.createTreeMap("trades_key_want").comparator(Fun.COMPARATOR).makeOrGet();

			// BIND
			Bind.secondaryKey(map, this.wantKeyMap,
					new Fun.Function2<Tuple3<String, Long, Tuple2<BigInteger, BigInteger>>, Tuple2<BigInteger, BigInteger>, Trade>() {
						@Override
						public Tuple3<String, Long, Tuple2<BigInteger, BigInteger>> run(
								Tuple2<BigInteger, BigInteger> key, Trade value) {
							...
							return new Tuple3<String, Long, Tuple2<BigInteger, BigInteger>>(wantKey,
									Long.MAX_VALUE - value.getTimestamp(), key);
						}
					});

			//
			this.haveKeyMap = database.createTreeMap("trades_key_have").comparator(Fun.COMPARATOR).makeOrGet();

			// BIND
			Bind.secondaryKey(map, this.haveKeyMap,
					new Fun.Function2<Tuple3<String, Long, Tuple2<BigInteger, BigInteger>>, Tuple2<BigInteger, BigInteger>, Trade>() {
						@Override
						public Tuple3<String, Long, Tuple2<BigInteger, BigInteger>> run(
								Tuple2<BigInteger, BigInteger> key, Trade value) {
							...
							return new Tuple3<String, Long, Tuple2<BigInteger, BigInteger>>(haveKey,
									Long.MAX_VALUE - value.getTimestamp(), key);
						}
					});

			// REVERSE KEY
			this.reverseKeyMap = database.createTreeMap("trades_key_reverse").comparator(Fun.COMPARATOR).makeOrGet();

			// BIND REVERSE KEY
			Bind.secondaryKey(map, this.reverseKeyMap,
					new Fun.Function2<Tuple2<BigInteger, BigInteger>, Tuple2<BigInteger, BigInteger>, Trade>() {
						@Override
	...
	private Collection<Tuple2> getKeys(Order order) {
		...
		// FILTER ALL KEYS
		Collection<Tuple2> keys = ((BTreeMap<Tuple2, Order>) uncastedMap)
				.subMap(Fun.t2(order.getId(), null), Fun.t2(order.getId(), Fun.HI())).keySet();
	...
	public SortableList<Tuple2<BigInteger, BigInteger>, Trade> getTrades(Order order) {
		// ADD REVERSE KEYS
		Collection<Tuple2<BigInteger, BigInteger>> keys = ((BTreeMap<Tuple2, Tuple2<BigInteger, BigInteger>>) this.reverseKeyMap)
				.subMap(Fun.t2(order.getId(), null), Fun.t2(order.getId(), Fun.HI())).values();
	...
	public List<Trade> getTrades(long haveWant) {
		...
		Collection<Tuple2<BigInteger, BigInteger>> keys = ((BTreeMap<Tuple3, Tuple2<BigInteger, BigInteger>>) this.haveKeyMap)
				.subMap(Fun.t3(haveKey, null, null), Fun.t3(haveKey, Fun.HI(), Fun.HI())).values();
		...
		keys = ((BTreeMap<Tuple3, Tuple2<BigInteger, BigInteger>>) this.wantKeyMap)
				.subMap(Fun.t3(wantKey, null, null), Fun.t3(wantKey, Fun.HI(), Fun.HI())).values();
	...
	public SortableList<Tuple2<BigInteger, BigInteger>, Trade> getTradesSortableList(long have, long want) {
		...
		// FILTER ALL KEYS
		Collection<Tuple2<BigInteger, BigInteger>> keys = ((BTreeMap<Tuple3, Tuple2<BigInteger, BigInteger>>) this.pairKeyMap)
				.subMap(Fun.t3(pairKey, null, null), Fun.t3(pairKey, Fun.HI(), Fun.HI())).values();
