...
public class OrderMap extends DbMap<BigInteger, Order> {
	...
	public OrderMap(QoraDb databaseSet, DB database) {
	...
	private Map<BigInteger, Order> openMap(DB database) {
		// OPEN MAP
		BTreeMap<BigInteger, Order> map = database.createTreeMap("orders").valueSerializer(new OrderSerializer())
				.makeOrGet();

		// HAVE/WANT KEY
		this.haveWantKeyMap = database.createTreeMap("orders_key_have_want").comparator(Fun.COMPARATOR).makeOrGet();
		...
		Bind.secondaryKey(map, this.haveWantKeyMap,
				new Fun.Function2<Tuple4<Long, Long, BigDecimal, BigInteger>, BigInteger, Order>() {
					...
					public Tuple4<Long, Long, BigDecimal, BigInteger> run(BigInteger key, Order value) {
						return new Tuple4<Long, Long, BigDecimal, BigInteger>(value.getHave(), value.getWant(),
								value.getPrice(), key);
					}
				});

		// HAVE/WANT KEY
		this.wantHaveKeyMap = database.createTreeMap("orders_key_want_have").comparator(Fun.COMPARATOR).makeOrGet();
		...
		Bind.secondaryKey(map, this.wantHaveKeyMap,
				new Fun.Function2<Tuple4<Long, Long, BigDecimal, BigInteger>, BigInteger, Order>() {
					@Override
					public Tuple4<Long, Long, BigDecimal, BigInteger> run(BigInteger key, Order value) {
						return new Tuple4<Long, Long, BigDecimal, BigInteger>(value.getWant(), value.getHave(),
								value.getPrice(), key);
					}
				});
	...
	private Collection<BigInteger> getKeys(long have, long want) {
		...
		// FILTER ALL KEYS
		Collection<BigInteger> keys = ((BTreeMap<Tuple4, BigInteger>) this.haveWantKeyMap)
				.subMap(Fun.t4(have, want, null, null), Fun.t4(have, want, Fun.HI(), Fun.HI())).values();
	...
	private Collection<BigInteger> getKeysHave(long have) {
		...
		// FILTER ALL KEYS
		Collection<BigInteger> keys = ((BTreeMap<Tuple4, BigInteger>) this.haveWantKeyMap)
				.subMap(Fun.t4(have, null, null, null), Fun.t4(have, Fun.HI(), Fun.HI(), Fun.HI())).values();
	...
	private Collection<BigInteger> getKeysWant(long want) {
		...
		// FILTER ALL KEYS
		Collection<BigInteger> keys = ((BTreeMap<Tuple4, BigInteger>) this.wantHaveKeyMap)
				.subMap(Fun.t4(want, null, null, null), Fun.t4(want, Fun.HI(), Fun.HI(), Fun.HI())).values();
	...
	public SortableList<BigInteger, Order> getOrdersWantSortableList(long want) {
		// FILTER ALL KEYS
		Collection<BigInteger> keys = ((BTreeMap<Tuple4, BigInteger>) this.haveWantKeyMap)
				.subMap(Fun.t4(null, want, null, null), Fun.t4(Fun.HI(), want, Fun.HI(), Fun.HI())).values();
