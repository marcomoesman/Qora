...
public class CancelOrderMap extends DbMap<byte[], Order> {
	...
	public CancelOrderMap(QoraDb databaseSet, DB database) {
	...
	protected Map<byte[], Order> getMap(DB database) {
		// OPEN MAP
		return database.createTreeMap("cancelOrderOrphanData").keySerializer(BTreeKeySerializer.BASIC)
				.valueSerializer(new OrderSerializer()).comparator(UnsignedBytes.lexicographicalComparator())
				.makeOrGet();
	}
