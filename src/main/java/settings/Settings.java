...
public class Settings {
	...
	public List<Peer> getKnownPeers() {
		try {
			boolean loadPeersFromInternet = (Controller.getInstance().getToOfflineTime() != 0L
	...
	public List<Peer> getKnownPeersFromInternet() {
		try {
			...
			if (this.cacheInternetPeers.size() == 0
					|| NTP.getTime() - this.timeLoadInternetPeers > 24 * 60 * 60 * 1000) {
				...
				String stringInternetSettings = IOUtils.toString(in, StandardCharsets.UTF_8);
			...
			LOGGER.info(Lang.getInstance().translate("Peers loaded from Internet with errors : ")
					+ this.cacheInternetPeers.size());
			return this.cacheInternetPeers;
		}
	}
