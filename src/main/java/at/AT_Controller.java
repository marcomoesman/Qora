...
public abstract class AT_Controller {
	...
	public static int checkCreationBytes( byte[] creation , String type, long fee, int height, int forkHeight, QoraDb db ) throws AT_Exception{
		if ( creation == null )
	...
	public static AT_Block getCurrentBlockATs( int freePayload , int blockHeight ){
		...
		QoraDb fork = QoraDb.getInstance();
	...
	public static AT_Block validateATs( byte[] blockATs , int blockHeight , QoraDb dbSet ) throws NoSuchAlgorithmException, AT_Exception {
	...
	//platform based 
	private static Tuple2<Long, Long> makeTransactions( AT at , QoraDb dbSet, int height, int seq, boolean yes) {
		long totalAmount = 0;
		long totalFees = 0;
	...
	{
		return getATAccountBalance( id , QoraDb.getInstance() );
	}

	private static long getATAccountBalance( byte[] id , QoraDb dbSet)
	{
