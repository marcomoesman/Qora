...
public class AT_Machine_State {
	...
	public final class Machine_State {
		...
		final TreeSet<Integer> jumps = new TreeSet<Integer>();
		...
		protected byte[] getMachineStateBytes() {
			final ByteBuffer bytes = ByteBuffer.allocate(getSize());
			bytes.order(ByteOrder.LITTLE_ENDIAN);

			flags[0] = (byte) ((running ? 1 : 0) | (stopped ? 1 : 0) << 1 | (finished ? 1 : 0 << 2)
					| (dead ? 1 : 0) << 3);
		...
		private void setMachineState(final byte[] machineState) {
			final ByteBuffer bf = ByteBuffer.allocate(getSize());
	...
	public AT_Machine_State(final byte[] atId, final byte[] creator, final short version, final byte[] stateBytes, final int csize, final int dsize,
			final int c_user_stack_bytes, final int c_call_stack_bytes, final int creationBlockHeight, final int sleepBetween,
			final long minActivationAmount, final byte[] apCode) {
		this.atID = atId;
	...
	public AT_Machine_State(final byte[] atId, final byte[] creator, final byte[] creationBytes, final int height) {
		this.version = AT_Constants.getInstance().AT_VERSION(height);
		this.atID = atId;
		this.creator = creator;

		final ByteBuffer b = ByteBuffer.allocate(creationBytes.length);
		...
		final int pageSize = (int) AT_Constants.getInstance().PAGE_SIZE(height);
	...
	protected void addTransaction(final AT_Transaction tx) {
		if (tx != null) {
			transactions.add(tx);
		}
	}
	...
	protected void setC_call_stack_bytes(final int c_call_stack_bytes) {
		this.c_call_stack_bytes = c_call_stack_bytes;
	}

	protected void setC_user_stack_bytes(final int c_user_stack_bytes) {
		this.c_user_stack_bytes = c_user_stack_bytes;
	}
	...
	protected void setG_balance(final Long g_balance) {
		this.g_balance = g_balance;
	}

	protected void setP_balance(final Long p_balance) {
		this.p_balance = p_balance;
	}

	protected void setMachineState(final Machine_State machineState) {
		this.machineState = machineState;
	}

	protected void setWaitForNumberOfBlocks(final int waitForNumberOfBlocks) {
	...
	protected void setMinActivationAmount(final long minActivationAmount) {
		this.minActivationAmount = minActivationAmount;
	}
	...
	public byte[] getState() {
		final byte[] stateBytes = machineState.getMachineStateBytes();
		final byte[] dataBytes = ap_data.array();

		final ByteBuffer b = ByteBuffer.allocate(getStateSize());
		b.order(ByteOrder.LITTLE_ENDIAN);
	...
	public void setState(final byte[] state) {
		final ByteBuffer b = ByteBuffer.allocate(state.length);
		b.order(ByteOrder.LITTLE_ENDIAN);
		b.put(state);
		b.flip();

		final int stateSize = this.machineState.getSize();
		final byte[] machineState = new byte[stateSize];
		...
		final byte[] apData = new byte[b.capacity() - b.position()];
		b.get(apData);
	...
	protected void setFreeze(final boolean freeze) {
